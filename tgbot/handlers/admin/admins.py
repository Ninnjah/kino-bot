...
async def add_request(m: Message, l10n: FluentLocalization, state: FSMContext):
    await state.clear()
    await m.answer(l10n.format_value("admin-admins-add-request-text"), reply_markup=cancel_kb.get(l10n))
    await state.set_state(AddAdminSG.user_id)


@router.message(StateFilter(AddAdminSG.user_id))
async def add_handler(m: Message, l10n: FluentLocalization, repo: Repo, state: FSMContext):
    try:
        ...
        await m.reply(l10n.format_value("error-user-id-is-invalid"), reply_markup=cancel_kb.get(l10n))
        return

    else:
        # If message from group or channel
        if user_id < 0:
            await m.reply(l10n.format_value("error-is-not-user-id"), reply_markup=cancel_kb.get(l10n))
            return

        # If user is not an admin
        elif not await repo.is_admin(user_id):
            await m.answer(
                l10n.format_value("admin-admins-add-handler-confirm", dict(user_id=user_id)),
...
@router.callback_query(StateFilter(AddAdminSG.confirm), yesno_kb.YesNoCallback.filter(F.action == True))
async def add_conf(
...
@router.callback_query(StateFilter(AddAdminSG.confirm), yesno_kb.YesNoCallback.filter(F.action == False))
async def add_reject(callback: CallbackQuery, l10n: FluentLocalization, state: FSMContext):
    with suppress(TelegramForbiddenError, TelegramBadRequest):
        await callback.message.delete()

    await callback.message.answer(l10n.format_value("admin-admins-add-conf-reject"), reply_markup=main_kb.get(l10n))
    await state.clear()
...
async def del_request(m: Message, l10n: FluentLocalization, state: FSMContext):
    await state.clear()
    await m.answer(l10n.format_value("admin-admins-del-request-text"), reply_markup=cancel_kb.get(l10n))
    await state.set_state(DelAdminSG.user_id)


@router.message(StateFilter(DelAdminSG.user_id))
async def del_handler(m: Message, l10n: FluentLocalization, repo: Repo, state: FSMContext):
    try:
        # If message is forwarded take user id from it
        if getattr(m, "forward_from", None):
            user_id = m.forward_from.id
        ...
        # If user is not an admin
            ...
            await m.answer(
                l10n.format_value("admin-admins-del-handler-confirm", dict(user_id=user_id)),
                reply_markup=yesno_kb.get(l10n, user_id)
            )
...
@router.callback_query(StateFilter(DelAdminSG.confirm), yesno_kb.YesNoCallback.filter(F.action == True))
async def del_conf(
        callback: CallbackQuery,
        callback_data: yesno_kb.YesNoCallback,
        l10n: FluentLocalization,
...
@router.callback_query(StateFilter(DelAdminSG.confirm), yesno_kb.YesNoCallback.filter(F.action == False))
async def del_reject(callback: CallbackQuery, l10n: FluentLocalization, state: FSMContext):
    with suppress(TelegramForbiddenError, TelegramBadRequest):
        await callback.message.delete()

    await callback.message.answer(l10n.format_value("admin-admins-del-conf-reject"), reply_markup=main_kb.get(l10n))
    await state.clear()
